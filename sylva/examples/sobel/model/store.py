...
def main():
    ...
    parser.add_argument('--out-mem', required=False, help="Path to output memory JSON file")
    args = parser.parse_args()

    ''' main '''
    global_image = load_all(args.global_image)
    input_data = load_all(args.in_mem)
    global_image[(OFFSET * CHUNK_SIZE):(OFFSET * CHUNK_SIZE + len(input_data))] = input_data 
    write_data(global_image, len(global_image), args.global_image)
