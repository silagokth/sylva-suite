...
def gx(img):
    '''Gx convolution for channel 0'''
    kernel = np.array([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]]
                      )  # Sobel kernel for Gx
    gx = np.array([[0 for j in range(img.shape[1])]
                  for i in range(img.shape[0])], dtype=np.int32)
    for i in range(1, img.shape[0] - 1):
        for j in range(1, img.shape[1] - 1):
            gx[i][j] = np.sum(np.multiply(
...
def main():
    ...
    gx_image = gx(image)
    gx_data = encode_image(gx_image)
    write_data(gx_data, OUT_TOKEN_SIZE * CHUNK_SIZE, args.out_mem)
