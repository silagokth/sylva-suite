...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // first copy: keep original
    output.line.extend(input.line.iter().cloned());
    
    let offset = input.line.len() as i64;

    // second copy: shifted addresses
    for l in input.line.iter() {
        output.line.push(Memory {
            address: l.address + offset,
            value: l.value.clone(),
        });
    }
